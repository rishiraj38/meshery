...
{{% alert color="info" title="Registry API paths" %}}Operations on Meshery's capabilities registry (models, components, relationships, categories, registrants, policies, and connection definitions) are canonically served under <code>/api/registry</code>. The legacy <code>/api/meshmodels</code> and <code>/api/meshmodel</code> routes remain available as deprecated aliases of the same operations so existing clients keep working.{{% /alert %}}
